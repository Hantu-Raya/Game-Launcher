...
#  CS2 settings - set to -1 to use default/auto
$force_width     = -1
$force_height    = -1
$force_refresh   = -1
$force_settings  = 1
...
#$extra_launch_options+= '-noreflex -noantilag'                      ##  uncomment if frametime issues
...
$do_not_set_desktop_res_to_match_game = 1
...
if ($do_not_set_desktop_res_to_match_game -le 0) {
  ...
  try {
    # AveYo: This advanced watcher handles Alt-Tab resolution switching
  ...
  # FALLBACK: If the C# watcher failed, we wait for the actual PROCESS to close.
  # This ensures resolution restores even if the Registry watcher didn't catch it.
  ...
  # --- PASSIVE MODE ---
  # Active switching is disabled, BUT we must wait for the game to exit
  # so we can restore the resolution if the game forced a change.
  ...
  if (Get-Process -Name $APPNAME -ErrorAction SilentlyContinue) {
      ...
      write-host " Game process not found after launch." -fore Red
  }
}
      ...
      _bookmon.EventReceived += (s, e) => {
        ...
        if (_started && _running == 1)
        {
          ...
          if (lpT == title) {
            ...
            _lastevt = 1; Change(verbose, def_scr, cfg_w, cfg_h, cfg_r > 0 ? cfg_r : def_refresh, 0);
          }
